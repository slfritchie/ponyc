...
/*
 * Arbitrary constants to name the thread/actor category for message
 * queue operations.
 */
...
/* Runtime code should not be calling the underlying ponyint_messageq_
 * functions directly.  Purposes:
...
#define ACTOR_MESSAGEQ_PUSH_SINGLE(sched, from_actor, to_actor, q, first, last) \
    ponyint_messageq_push_single(Q_TYPE_ACTOR, (uintptr_t) (sched), \
      (uintptr_t) (from_actor), (uintptr_t) (to_actor), (q), (first), (last))
#define ACTOR_MESSAGEQ_POP(sched, actor, q) \
    ponyint_messageq_pop(Q_TYPE_ACTOR, (uintptr_t) (sched), (uintptr_t) (actor), (q))
#define THREAD_MESSAGEQ_PUSH(from_index, to_index, q, first, last) \
    ponyint_messageq_push(Q_TYPE_THREAD, 0, \
      (uintptr_t) (from_index), (uintptr_t) (to_index), (q), (first), (last))
#define THREAD_MESSAGEQ_PUSH_SINGLE(from_index, to_index, q, first, last) \
